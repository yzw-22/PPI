"""Run the 10-epoch SHS27k/bfs alternating-training experiment."""
...
def evaluate(trainer, node_features, split_graph, targets, labels, batch_size):
    """Evaluate greedy sampled subgraphs and return multi-label metrics."""
...
def run(args):
    ...
    train_graph = graph.build_graph("train", remap_nodes=False, undirected=True)
    test_graph = graph.build_graph("test", remap_nodes=False, undirected=True)
...
def parse_args():
    ...
    parser.add_argument("--output", default="shs27k_bfs_10epoch.json")
