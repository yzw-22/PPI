...
- **同一个蛋白对可能有多行**（不同 `mode`），需按 `(item_id_a, item_id_b)` 分组做 **OR 聚合**
- 蛋白对是无向的（排序后去重）
...
- **BFS/DFS**：train 和 val/test 来自图中不同连通区域，用于评估结构偏移下的泛化能力
...
- `id` 格式：`ENSEMBL_A-ENSEMBL_B`（用 `-` 分割）
- `seq_a` / `seq_b`：完整氨基酸序列
...
### 各数据集 Label 分布（按 actions.txt 中 mode 出现次数）

| Label | SHS27k | SHS148k | STRING |
|-------|--------|---------|--------|
| reaction | 6,328 | 102,964 | 1,669,750 |
| binding | 8,034 | 93,632 | 1,610,314 |
| catalysis | 6,984 | 67,168 | 998,266 |
| activation | 6,594 | 42,516 | 232,240 |
| inhibition | 2,814 | 34,712 | 147,676 |
| ptmod | 2,606 | 20,154 | 88,424 |
| expression | 1,374 | 7,896 | 28,484 |
...
|--------|:-----:|:--------:|:--------:|:--------------:|:---------------:|
| SHS27k | 1,690 | 7,624 | 409 | **14** | 512 |
| SHS148k | 5,189 | 44,488 | 1,016 | **6** | 1,244 |
...
| SHS27k | bfs | 4,562 | 9 |
| SHS27k | dfs | 4,571 | 12 |
| SHS27k | random | 4,574 | 10 |
| SHS148k | dfs | 26,599 | 20 |
| SHS148k | random | 26,692 | 4 |
| STRING | dfs | 355,891 | 4 |

**BFS/DFS 划分**：训练和 val/test 取自不同连通分量，孤立 PPI 必然隔离在训练分量中，不会出现在 val/test 中。Random 划分下孤立 PPI 可能被分配到 val/test，但概率较低（训练集占 60%）。
...
**关键结论**：
- **BFS/DFS**："两节点均在训练集"恒为 0（设计使然——train/test 来自不同连通分量）。70-87% 的测试 PPI 有一个节点在训练集中出现过（跨分量边），12-30% 的两个节点都从未出现。
